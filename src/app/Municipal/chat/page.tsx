...
interface ChatThread {
    ...
    complaint?: { id: string; issueType: string } | null;
}

export default function MunicipalChatPage() {
                    ...
                    <div className="w-80 bg-white border-r border-border-light flex flex-col h-full z-10 shadow-soft">
                        <div className="p-4 border-b border-border-light flex items-center justify-between">
                            <h2 className="font-heading font-bold flex items-center gap-2">
                                <Link href="/Municipal" className="text-text-secondary hover:text-brand-primary">
                                            ...
                                            {thread.complaint && (
                                                <div className="flex items-center gap-1 mb-1">
                                                    <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-amber-100 text-amber-700 rounded-full text-[10px] font-semibold">
                                                        ...
                                                        {thread.complaint.issueType} #{thread.complaint.id.slice(-6).toUpperCase()}
                                                    </span>
                                                </div>
                                            )}
                        ...
                        {activeThreadId ? (
                            <>
                                ...
                                <div className="px-6 py-4 border-b border-border-light bg-white flex items-center gap-4 shadow-sm">
                                    ...
                                    {activeThread?.complaint && (
                                        <Link
                                            href={`/Municipal/complaints?highlight=${activeThread.complaint.id}`}
                                            className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg text-xs font-semibold hover:bg-amber-100 transition-colors"
                                        >
                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                            {activeThread.complaint.issueType} #{activeThread.complaint.id.slice(-6).toUpperCase()}
                                        </Link>
                                    )}
                                </div>
                                ...
                                <div className="flex-grow overflow-y-auto p-6 space-y-4">
                                    {messages.map((m) => {
                                        const isMe = m.senderRole === 'municipal';
                                        return (
                            ...
                            <div className="flex-grow flex flex-col items-center justify-center text-text-secondary">
                                ...
                                <h3 className="font-heading font-bold text-lg mb-2">Municipal Citizen Inbox</h3>
